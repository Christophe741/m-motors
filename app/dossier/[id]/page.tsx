...
import { StatutDossier, StatutDocument, TypeDocument } from '@/lib/types';
...
const statutConfig: Record<StatutDossier, { icon: any; color: string; label: string }> = {
  brouillon: { icon: Clock, color: 'text-gray-500', label: 'Brouillon' },
  soumis: { icon: FileText, color: 'text-blue-500', label: 'Soumis' },
  en_cours: { icon: Clock, color: 'text-yellow-500', label: 'En cours de traitement' },
  valide: { icon: CheckCircle, color: 'text-green-500', label: 'Validé' },
  refuse: { icon: XCircle, color: 'text-red-500', label: 'Refusé' },
};

const documentStatutConfig: Record<StatutDocument, { icon: any; color: string; label: string }> = {
...
export default function DossierDetailPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.data?.vehicule_id) {
            const vehicleResponse = await fetch(`/api/vehicles/${result.data.vehicule_id}`);
  ...
  const timeline: { statut: StatutDossier; active: boolean }[] = [
    { statut: 'soumis', active: ['soumis', 'en_cours', 'valide', 'refuse'].includes(dossier.statut) },
    { statut: 'en_cours', active: ['en_cours', 'valide', 'refuse'].includes(dossier.statut) },
    { statut: dossier.statut === 'valide' ? 'valide' : 'refuse', active: ['valide', 'refuse'].includes(dossier.statut) },
  ];
          ...
          <div className="max-w-3xl mx-auto space-y-6">
              ...
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <CardDescription>Créé le {formatDate(dossier.date_creation)}</CardDescription>
                  </div>
                  <Badge
                    variant={
                    ...
                    return (
                      <div key={step.statut} className="flex gap-4">
                        <div className="flex flex-col items-center">
                          <div
                            className={`rounded-full p-2 ${
                              step.active ? 'bg-primary text-white' : 'bg-gray-200 text-gray-400'
                            }`}
                          >
                        ...
                        <div className="flex-1 pt-1">
                          <h4 className={`font-semibold ${step.active ? '' : 'text-muted-foreground'}`}>
                            {stepConfig.label}
                          </h4>
                          {step.active && step.statut === dossier.statut && (
                            <p className="text-sm text-muted-foreground mt-1">
                              Dernière mise à jour : {formatDate(dossier.date_modification)}
                            </p>
                          )}
                        </div>
                      </div>
                ...
                {dossier.commentaire_admin && (
                  <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="flex gap-2">
                      ...
                      <div>
                        <h4 className="font-semibold text-blue-900 mb-1">Message de l&apos;administrateur</h4>
                        <p className="text-sm text-blue-700">{dossier.commentaire_admin}</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {dossier.documents.length} document{dossier.documents.length > 1 ? 's' : ''} déposé{dossier.documents.length > 1 ? 's' : ''}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {dossier.documents.map((doc: any) => {
                    const docConfig = documentStatutConfig[doc.statut as StatutDocument];
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <h4 className="font-medium">
                                {documentLabels[doc.type_document as TypeDocument]}
                              </h4>
                              <Badge
                                variant={
                                  doc.statut === 'valide'
                            ...
                            <p className="text-sm text-muted-foreground">
                              {doc.fichier_nom} • Déposé le {formatDate(doc.date_upload)}
                            </p>
                            {doc.commentaire && (
                              <p className="text-sm text-red-600 mt-1 italic">{doc.commentaire}</p>
                            )}
                          </div>
                        </div>
                      </div>
